...
/**
 * Send email - completely safe, never throws
 * In development: logs to console
 * In production: sends real email if SMTP is configured, otherwise logs
 */
const sendEmail = async ({ to, subject, html }) => {
  try {
    ...
    // In production, try to send but don't fail if SMTP isn't configured
    if (!process.env.SMTP_HOST || !process.env.SMTP_USER) {
      console.log('📧 [PROD EMAIL] SMTP not configured. Would send to:', to);
    ...
    await transporter.sendMail({
      from: process.env.FROM_EMAIL || 'noreply@library.com',
      to,
      subject,
      html,
    });
